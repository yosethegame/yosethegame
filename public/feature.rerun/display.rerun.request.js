...
var tryLevelAtIndex = function(index, player, database, response, callback) {
	...
	request(requestSent, function(error, remoteResponse, content) {
		...
		checker.validate(requestSent, remoteResponse, content, function(status) {
			...
			if (index < levelsToTry.length-1) {
				tryLevelAtIndex(index + 1, player, database, response, callback);
			}
			else {
				callback();
			}
...
rerun = function(request, response, database, done) {
